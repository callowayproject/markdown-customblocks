...
from .utils import E, Fetcher, Markdown, PageInfo, image
...
def figure(ctx, url, *args, lightbox: bool = None, local: bool = None, embed: bool = None, thumb: bool = None, **kwds):
    ...
    if thumb is None or (thumb and thumbOption):
        thumb = thumbOption
    if lightbox is None:
    ...
    return E(
        ...
        E(
            "a",
            dict(href=f"#{id}" if lightbox else detailsrc, target=None if lightbox else "_blank"),
            E(
                "img" + (".thumb" if lightbox and thumb else ""),
                src=previewsrc,
                title=title,
                alt=alt,
            ),
        ...
        E("figcaption", Markdown(ctx.content, parser=ctx.parser)),
        **kwds,
    )


def linkcard(ctx, url: str, *args, wideimage: bool = True, **overrides):
    ...
    return E(
        ...
        info.image
        and E(
            ".linkcard-featured-image" + ("" if wideimage else ".side"),
            nl,
            E(
                "a",
                {"href": url, "target": "_blank"},
                nl,
                E("img", src=info.image),
                nl,
            ),
            nl,
        ),
        ...
        E(
            ".linkcard-excerpt",
            nl,
            Markdown(ctx.content) if ctx.content.strip() else E("p", nl, info.description, nl),
            nl,
            E("span.linkcard-more", E("a", "Read more", href=url, target="_blank")),
            nl,
        ),
        nl,
        E(
            ...
            E(
                ...
                E(
                    "a",
                    {"href": info.siteurl, "target": "_blank"},
                    nl,
                    E(
                        "img.linkcard-site-icon",
                        height="32",
                        src=info.siteicon,
                        width="32",
                    ),
                    nl,
                    E("span", info.sitename.upper()),
                    nl,
                ),
                nl,
            ),
...
def youtube(ctx, id, *args, autoplay=False, controls=True, loop=False, style=None, **kwds):
    ...
    wrapperStyle = (
        ("position:relative; padding-bottom:56.25%; height:0; overflow:hidden; width:100%") if inlineStyle else None
    )
    iframeStyle = ("position:absolute; top:0; left:0; width:100%; height:100%;") if inlineStyle else None
    if style:
        if wrapperStyle:
            wrapperStyle = "; ".join((wrapperStyle, style))
        else:
            wrapperStyle = style
...
def vimeo(ctx, id, *args, autoplay=False, loop=False, byline=True, portrait=False, **kwds):
    ...
    return E(
        ...
        E(
            "iframe",
            src="https://player.vimeo.com/video/{}?{}".format(id, "&".join(options)),
            width="100%",
            height="300",
            frameborder="0",
...
def peertube(
    ...
    **kwds,
    ...
    if autoplay:
        options.append("autoplay=1")
    if muted:
        options.append("muted=1")
    if not title:
        options.append("title=0")
    if not controls:
        options.append("controlBar=0")
    if not p2p:
        options.append("p2p=0")
...
def verkami(ctx, id, *, landscape=False):
    ...
    style = "width: 480px; height: 210px" if landscape else "width: 240px; height: 490px"
    return E(
        "iframe.js-widget-iframe",
...
def twitter(
    ...
    options += "&conversation=none" if not conversation else ""

    fetcher = Fetcher("fetchercache/twitter")
    oembedurl = f"https://publish.twitter.com/oembed?url=https://twitter.com/{user}/status/{tweet}&dnt=True{options}"
    response = fetcher.get(oembedurl)
    result = ns(response.json())
    soup = BeautifulSoup(result.html, "html.parser")
    return str(soup.find("blockquote"))
...
def facebook(ctx, page, post, *args, text=True, width="auto", height=452.0, **kwds):
...
def map(ctx, location=None, marker=True, *args, **kwds):
    ...
    bbox = "%2C".join(str(x) for x in (geocoding["bbox"]["northeast"][::-1] + geocoding["bbox"]["southwest"][::-1]))
